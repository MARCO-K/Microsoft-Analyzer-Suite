...
#>
    ...
    process {
        ...
        if ($Blacklist) {
            ...
            $supportlist += Import-SupportlistFromFolder -Folder $Blacklist -ListType 'Blacklist'
        }

        # import Whitelists
        if ($Whitelist) {
            Write-PSFMessage -Level 'Verbose' -Message "Importing Blacklist from: $Whitelist" -FunctionName $MyInvocation.MyCommand.Name
            $supportlist += Import-SupportlistFromFolder -Folder $Whitelist -ListType 'Whitelist'
        }
        ...
        Function Start-Processing {
            ...
            Function Get-Stats {
                ...
                $AppDisplayName = $Results | Group-Object AppId, AppDisplayName | Select-Object @{Name = 'AppDisplayName'; Expression = { $_.Values[0] } }, @{Name = 'AppId'; Expression = { $_.Values[1] } }, Count, @{Name = 'PercentUse'; Expression = { "{0:p2}" -f ($_.Count / $Total) } } | Sort-Object Count -Descending
                ...
                if ($AppDisplayName.Count -gt 0) {
                    switch ($outputFormat) {
                        ...
                        'XLSX' {
                            Export-MSAExcel -exportdata $AppDisplayName -name 'Application' -ID 'AppId'
                        }
                        'JSON' {
